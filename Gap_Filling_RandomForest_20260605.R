...
merged_la2 # This data frame comes from merging R script

flux_data<-read.csv("Data/US_LA3_mean_flux_clean_2019_2023.csv")
#flux_data<-read.csv("Data/US_LA2_mean_flux_clean_2021_2023.csv")
names(flux_data)

flux_data$Date<-ymd(flux_data$date)
range(flux_data$Date,na.rm=TRUE)
range(flux_data$FCH4mean,na.rm=TRUE)
range(flux_data$FCmean,na.rm=TRUE)
...
#flux_var_la2<-left_join(flux_data %>% select(-date),merged_la2,by = "Date") %>% filter(Date >= "2021-01-01")
#flux_var_la2$DOY<-yday(flux_var_la2$Date)

colnames(flux_var_la3)
str(flux_var_la3)

ggplot(flux_var_la3, aes(x = Date, y = FCH4mean)) + geom_line() +  
  ylab(expression(n * mol ~ CH[4] ~ m^{-2} ~ day^{-1})) + theme_light() + ggtitle("EC Tower: LA2", subtitle = "Methane") # 3x6
ggplot(flux_var_la3, aes(x = Date, y = FCmean)) + geom_line() + 
  ylab(expression(mu * mol ~ CO[2] ~ m^{-2} ~ day^{-1})) + theme_light() + ggtitle("EC Tower: LA2", subtitle = "Carbon Dioxide") # 3x6
ggplot(flux_var_la3, aes(x = Date, y = LEmean)) + geom_line() + 
  ylab(expression(W ~ m^{-2} ~ day^{-1})) + theme_light() + ggtitle("EC Tower: LA2", subtitle = "Latent Heat") # 3x6


##### CO2 Gap Filling #####
# Step 1: Filter out complete cases for FC
df_complete <- flux_var_la3 %>% filter(!is.na(FCmean)) %>% select(-c(FCH4mean, LEmean))
head(df_complete)
...
model <- randomForest(
  FCmean ~ .,
  data = train_data %>% select(-c(Date)),  # exclude datetime if needed
  na.action = na.omit,
  importance = TRUE
)

# Step 4: Validate on the 30% test set
test_predictors <- test_data %>% select(-c(Date))
...
df_missing_clean <- flux_var_la3 %>% filter(is.na(FCmean)) %>% select(-c(Date))
...
ggplot() +
  ...
  labs(
    ...
    x = "Date",
    y = expression(umol~CO[2]~m^{-2}~s^{-1}),
    color = "Legend"
  ) +
...
##### LE Gap Filling #####
# Step 1: Filter out complete cases for LE
df_complete <- flux_var_la3 %>% filter(!is.na(LEmean)) %>% select(-c(FCH4mean, FCmean))
head(df_complete)
# Step 2: Split into 70% train / 30% test
set.seed(42)  # for reproducibility
train_index <- sample(nrow(df_complete), 0.7 * nrow(df_complete))
...
# Step 4: Validate on the 30% test set
test_predictors <- test_data %>% select(-c(Date))
test_actuals <- test_data$LEmean
range(test_data$LEmean)
predicted_test <- predict(model, newdata = test_predictors)
...
# Filter rows where LE is missing 
df_missing_clean <- flux_var_la3 %>% filter(is.na(LEmean)) %>% select(-c(Date))
na_days<-length(df_missing_clean$LEmean)
head(df_missing_clean)

# Predict missing FC values
LE_predicted <- predict(model, newdata = df_missing_clean)
head(LE_predicted)
...
# Add predictions back to original data
flux_var_la3$LE_filled <- flux_var_la3$LEmean
flux_var_la3$LE_filled[is.na(flux_var_la3$LEmean)] <- LE_predicted
length(flux_var_la3$LE_filled[is.na(flux_var_la3$LEmean)])
length(LE_predicted)
...
ggplot() +
  ...
  geom_line(
    data = flux_var_la3 ,
    aes(x = Date, y = LE_filled, color = "Filled"),
    na.rm = FALSE
  ) +
  # Original data (black), NA breaks the line
  geom_line(data = flux_var_la3, aes(x = Date, y = LEmean, color = "Original"), na.rm = FALSE) +
  scale_color_manual(values = c("Original" = "black", "Filled" = "red")) +
  # X axis with major ticks yearly, minor ticks monthly
  scale_x_datetime(
    date_breaks = "6 months",       # major tick every year
  ...
  annotate(
    "text",
    x = min(flux_var_la3$Date, na.rm = TRUE),
    y = max(flux_var_la3$LE_filled, na.rm = TRUE),
    hjust = 0,
    vjust = 1,
    size = 4,
    label = paste0(
  ...
  labs(
    title = "Actual vs Filled Latent Heat Over Time",
    x = "Date",
    y = expression(W~m^{-2}),
    color = "Legend"
  ) +
...
summary(flux_var_la3$LE_filled)
...
#df_complete <- flux_var_la3 %>% filter(!is.na(FCH4mean)) %>% select(-c(FCmean,LEmean))
df_complete <- flux_var_la3 %>% filter(!is.na(FCH4mean)) %>% select(-c(FCmean,LEmean, FCH4_filled))
head(df_complete)
# Step 2: Split into 70% train / 30% test
set.seed(42)  # for reproducibility
train_index <- sample(nrow(df_complete), 0.7 * nrow(df_complete))
...
model <- randomForest(
  FCH4mean ~ .,
  data = train_data %>% select(-Date),  # exclude datetime if needed
  na.action = na.omit,
  importance = TRUE
)
...
df_missing_clean <- flux_var_la3 %>% filter(is.na(FCH4mean)) %>% select(-c(Date, FCmean, FCH4_filled))
...
#FCH4_filled_Only<-flux_var_la3$FCH4_filled
#FCH4_filled_wFC<-flux_var_la3$FCH4_filled
#FCH4_filled_wFC_wLE<-flux_var_la3$FCH4_filled

flux_var_la3$FCH4_filled<-FCH4_filled_Only
flux_var_la3$FCH4_filled_wFC<-FCH4_filled_wFC
flux_var_la3$FCH4_filled_wFC_wLE<-FCH4_filled_wFC_wLE
head(flux_var_la3)
